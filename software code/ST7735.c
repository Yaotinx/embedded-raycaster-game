...
void ST7735_DrawTransparentmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
  int16_t skipC = 0;
  int16_t originalWidth = w;
  int i = w*(h - 1);

  // --- KEEP YOUR CLIPPING LOGIC EXACTLY AS IS ---
  ...
  // Save the starting point because the loop counters (x, y) will overwrite them
  int16_t startX = x;
  int16_t startY = y;
  ...
  for(y=0; y<h; y++){
    for(x=0; x<w; x++){
      ...
      if(color != 0){ 
        // DrawPixel is the ONLY way to skip black without rainbow static
        ST7735_DrawPixel(startX + x, startY - h + 1 + y, color);
      }
      i = i + 1;
    }
    i = i + skipC;
